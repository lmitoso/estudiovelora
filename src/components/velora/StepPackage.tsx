  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {combos.map((combo, i) => (
          <motion.button
            ...
            className={`p-4 rounded-lg border text-center relative transition-all ${
          ...
          >
            {combo.popular && (
              <span className="absolute -top-2 left-1/2 -translate-x-1/2 text-[10px] font-body font-medium bg-primary text-primary-foreground px-2 py-0.5 rounded-full">
                Mais vendido
              </span>
            )}
            <h4 className="font-display text-lg text-foreground">{combo.label}</h4>
            <p className="text-xs text-muted-foreground font-body mt-1">
              {combo.photos} fotos {combo.videos > 0 && `+ ${combo.videos} vídeo${combo.videos > 1 ? "s" : ""}`}
            </p>
            <p className="text-2xl font-display font-semibold text-primary mt-2">
              R${combo.price}
            </p>
      ...
      <div className="text-center">
        <button onClick={handleCustom} className="text-xs text-muted-foreground font-body underline hover:text-primary transition-colors">
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground">(R${PHOTO_PRICE}/und)</span>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground">(R${VIDEO_PRICE}/und)</span>
            </div>
            <div className="flex items-center gap-3">
              <button
      ...
      <div className="bg-card border border-primary/20 rounded-lg p-4 text-center">
        <p className="text-xs text-muted-foreground font-body">Total do investimento</p>
        <p className="text-4xl font-display font-bold velora-text-gradient mt-1">
          R${total}
        </p>
      ...
      <div className="flex justify-between pt-2">
        ...
        <motion.button
          ...
          disabled={total === 0}
          onClick={onGenerate}
          className="velora-gradient px-8 py-3 rounded-lg font-body text-sm font-medium text-primary-foreground disabled:opacity-30 animate-pulse-gold"
        >
          ✨ Gerar Campanha
        </motion.button>
      </div>
